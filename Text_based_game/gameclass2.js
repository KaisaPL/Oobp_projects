...
class Player extends Characters {
    ...
    attack(enemy){
        ...
        if(strike > this.hitChance){
            console.log('Your hit misses and the creature comes closer to you!');
        }else{
            ...
            if(enemy.hitPoints <= 0){
                ...
                console.log(enemy.name+ " has the hp of "+enemy.hitPoints);
            }
        }
    }
    look(){
    ...
    move(room){
        ...
        if(world[this.currentRoom].exit == true){
            console.log('You reached the exit. You won the game!')
            gameRunning = false;
        }
        for(let i = 0; i < world[this.currentRoom].creatures.length; i++){
...
let entrance = new Rooms('Entrance', 'You are in the entrance of the dungeon. \nYou can see one door leading deeper into the dungeon. What do you do?', false);
let hallway = new Rooms('Hallway', 'You are now in the dark hallway. What do you do?', false);
let chamber = new Rooms('Chamber', 'You arrive in the chamber. It is very dark and the air feels intimidating. What do you do?', false);
let portal = new Rooms('Portal', 'You enter the portal and you leave the dungeon.\nCongratulations, you survived!', true);
...
let player = new Player(10, 75, 2, 'Kaisa');
